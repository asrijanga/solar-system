...
- `npm run alloc` measures allocation in the interactive frame loop over 600 frames. CI runs it in its own job: on a pull request only when the per-frame code changed (`src/main.ts`, `src/scenes`, `src/core`, `src/gpu`, `tools/perf`, the package files), and on every push to main (owner, 2026-09-26).
...
- **Never invent data.** No procedural noise, guessed coordinates, or gap-filling. A missing value is shown as missing. One owner-approved exception (2026-09-25, docs/stories/SS-10b.md): the labelled approximation below the finest measurement in local mode. It is off by default and labelled on screen while on. It never changes a measured value (it passes exactly through every sample). It is never used in a checked capture, and its statistics come from cited sources or measured data. Nothing else may use it.
...
- **Gather first, then build.** Before building or rebuilding a world, inventory every available dataset for it (every mission and instrument: coverage, resolution, wavelength, frame, known artefacts, licence) in `docs/data/<world>.md`. Then recreate its terrain and features as real geometry (elevation at the finest resolution available) with true, unshaded albedo, so lighting works out of the box: terminator, shadows and phase emerge from geometry and photometry for any sun angle. Never bake lighting into a texture. The Moon is due its inventory and a rebuild from it.
