...
- **Real NASA maps of every world**, all cached in this repository. The page makes no third-party request at load: the fonts are self-hosted and the Sun's panel ships with a cached Solar Dynamics Observatory frame that is only upgraded to the live one if the network allows. Highlights: an 8192 x 4096 Lunar Reconnaissance Orbiter mosaic for the Moon, Cassini's infrared global mosaic for Titan, MESSENGER for Mercury, Magellan for Venus, Viking for Mars and its two moons, Cassini for Jupiter and Saturn, Voyager 2 for Neptune and Triton, and New Horizons for Pluto and Charon.
- **Where a spacecraft never looked, the map says so.** New Horizons passed Charon before its southern hemisphere left winter darkness, and Voyager 2 saw only part of Triton. Those gaps arrive as black in the source data. Rather than render a black gash, the imaged terrain is reflected across the coverage boundary so the gap carries plausible craters, and each info panel states how much of that world is extrapolated.
